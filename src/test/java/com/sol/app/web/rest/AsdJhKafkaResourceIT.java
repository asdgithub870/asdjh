...
import com.sol.app.config.KafkaProperties;
...
class AsdJhKafkaResourceIT {
    ...
    void setup() {
        ...
        AsdJhKafkaResource kafkaResource = new AsdJhKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/asd-jh-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/asd-jh-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
